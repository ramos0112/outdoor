                        ...
                        <tbody>
                            @foreach ($movilidades as $movilidad)
                                <tr>
                                    ...
                                    <td>{{ $movilidad->empresa }}</td>
                                    <td>{{ $movilidad->conductor }}</td>
                                        ...
                                        @else
                                            <ul class="mb-0 list-unstyled">
                                                @foreach ($movilidad->guias as $guia)
                                                    <li><i class="bi bi-person-fill"></i> {{ $guia->nombre }}
                                                        {{ $guia->apellido }}</li>
                                                @endforeach
                                            </ul>
                                        @endif
...
@section('js')
    ...
    $('#id_guia').select2({
        ...
        width: '100%',
        placeholder: 'Seleccione uno o más guías'
    });
});
            ...
            $('.modal').on('shown.bs.modal', function() {
                $(this).find('[id^="id_guia_edit"]').select2({
                    dropdownParent: $(this),
                    width: '100%'
                });
            }).on('hidden.bs.modal', function() {
                $(this).find('[id^="id_guia_edit"]').select2('destroy');
            });
